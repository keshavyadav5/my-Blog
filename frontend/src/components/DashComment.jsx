...
const DashComment = () => {
  ...
  useEffect(() => {
    const fetchComments = async () => {
      try {
        const res = await axios.get('http://localhost:3000/api/comment/getAllComment', {
  ...
  const handleShowMore = async () => {
    ...
    try {
      const res = await axios.get(`http://localhost:3000/api/comment/getAllComment?startIndex=${startIndex}`, {
        withCredentials: true
      });
  ...
  const handleDeleteComment = async () => {
    setShowModal(false);
    try {
      await axios.delete(`http://localhost:3000/api/comment/deleteComment/${commentIdToDelete}`, {
        withCredentials: true
      });
