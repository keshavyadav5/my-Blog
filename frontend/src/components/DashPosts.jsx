...
const DashPosts = () => {
  ...
  const fetchPosts = async () => {
    try {
      const res = await axios.get(
        `http://localhost:3000/api/post/getposts?userId=${currentUser?.rest?._id || currentUser?._id}&limit=9`
      );
  ...
  const handleShowMore = async () => {
    ...
    try {
      const res = await axios.get(
        `http://localhost:3000/api/post/getposts?userId=${currentUser?.rest?._id || currentUser?._id}&limit=9&startIndex=${startIndex}`
      );
  ...
  const deletePost = async (postId) => {
    try {
      const res = await axios.delete(`http://localhost:3000/api/post/deletepost/${postId}`, {
